...
function ensureRectangular(schema, rows) {
  ...
  for (let r = 0; r < widths.length; r++) {
    if (r >= rows.length)
      rows.push(import_prosemirror_model3.Fragment.empty);
    if (widths[r] < width) {
...
function arrow(axis, dir) {
  return (state, dispatch, view) => {
    ...
    if (axis == "horiz") {
      ...
      let newSel;
      if ($next)
        newSel = import_prosemirror_state5.Selection.near($next, 1);
      else if (dir < 0)
        newSel = import_prosemirror_state5.Selection.near(state.doc.resolve($cell.before(-1)), -1);
      else
        newSel = import_prosemirror_state5.Selection.near(state.doc.resolve($cell.after(-1)), 1);
...
function atEndOfCell(view, axis, dir) {
  if (!(view.state.selection instanceof import_prosemirror_state5.TextSelection))
    return null;
