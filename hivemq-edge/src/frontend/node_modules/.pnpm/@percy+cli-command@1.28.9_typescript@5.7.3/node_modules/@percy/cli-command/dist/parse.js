...
function isFlag(option) {
  ...
  // safe to say it's a flag without a space
  if (!option.includes(' ')) return true;
  // containing a space with no operator is not a proper flag
  if (!option.includes('=')) return false;
  // as long as the operator precedes the space, it is a flag
...
async function maybeParseCommand(input, parsed) {
  ...
  for (let name of names) {
    var _parent$definition$co, _command;
    let parent = command || parsed.command;

    // find any nested command by name
    ...
    if (typeof ((_command = command) === null || _command === void 0 ? void 0 : _command.definition.hidden) === 'string') {
...
async function maybeParseFlag(input, parsed) {
  ...
  // end of options and arguments, consume remaining input
  if (input[0] === '--') {
  ...
  let short = input[0][1] !== '-' && input[0][1];
  // get any value set with `=` and remove any `=` from short values
  let [name, value] = short ? ['', input[0].substr(2) || null] : input[0].split('=');
  value = short && (_value = value) !== null && _value !== void 0 && _value.startsWith('=') ? value.substr(1) : value;

  // the flag was negated
  let negated = name.startsWith('--no-');
  name = name.replace(/^--(no-)?/, '');

  // search for the provided short or long flag
  ...
  if (flag) {
    ...
    if (!flag.type || flag.type === 'boolean') {
      ...
      if (short && value) input.unshift(`-${value}`);

      // a value was provided to a boolean flag
      if (!short && value) {
...
async function maybeParseArg(input, parsed) {
  ...
  // the argument index needs to be tracked since flag options can precede argument options
  let index = parsed.__args_index__ ?? (parsed.__args_index__ = 0);

  // there are arguments left to parse
  if (index < ((_parsed$command$defin = parsed.command.definition.args) === null || _parsed$command$defin === void 0 ? void 0 : _parsed$command$defin.length)) {
    ...
    if (arg.parse) value = await arg.parse(value);

    // add this arg to the set of parsed operators
...
function validateInclusiveFlag(flag, parsed) {
  ...
  let invalid = flag.inclusive.reduce((invalid, name) => findFlag(name, parsed, flag =>
  // invalid options are options that have _not_ been provided
...
function validateExclusiveFlag(flag, parsed) {
  ...
  let invalid = flag.exclusive.reduce((invalid, name) => findFlag(name, parsed, flag =>
  // invalid options are options that _have_ been provided
  parsed.operators.has(flag) ? invalid.concat(flag) : invalid), []);
  if (invalid.length) {
    // show full flag usage for invalid options
    invalid = [flag, ...invalid].map(f => flagUsage(f)).join("', '");
...
function attributeName(option, parsed) {
  ...
  let value = parsed.operators.get(option) ?? option.default;
  // the option's attribute name might be contingent on it's value
  let name = typeof attribute === 'function' ? attribute(value) : attribute;
  // default the attribute name to the camelcase name or short character
  name || (name = option.name ? camelcase(option.name) : option.short);

  // deprecated options can be mapped to other options
  if (deprecated) {
    var _recommend;
    let [until, recommend] = Array.isArray(deprecated) ? deprecated : [typeof deprecated === 'string' ? deprecated : ''];

    // recommended replacement flag
...
export async function parse(command, argv) {
  ...
  for (let arg of parsed.command.definition.args ?? []) {
    ...
    if (parsed.operators.has(arg)) {
      ...
      if (arg.validate) await arg.validate(value, parsed);
