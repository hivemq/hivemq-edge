...
async function visibleArguments(command) {
  let args = command.definition.args ?? [];

  // not hidden and not deprecated
...
function flagDescription(flag) {
  ...
  // a boolean that defaults to false will be displayed with a --no prefix
...
async function visibleFlags(command) {
  let flags = command.definition.flags;
...
function formatOptionHelp(option, usageWidth, maxWidth) {
  ...
  // calculate the available description width, at minimum 40
  let descWidth = Math.max(maxWidth - usageWidth, 40);
  // pad usage up to the description and indent the description
...
export async function formatHelp(command) {
  ...
  // collect visible items first to determine max usage width
  let args = await visibleArguments(command);
  let commands = await visibleCommands(command);
  let flags = await visibleFlags(command);

  // maximum help output total width
  let maxWidth = logger.stdout.columns || 100;
  // calculate the longest usage width
  let usageWidth = Math.max(args.reduce((max, [usage]) => Math.max(max, usage.length), 0), commands.reduce((max, [usage]) => Math.max(max, usage.length), 0), flags.reduce((max, [usage]) => Math.max(max, usage.length), 0));

  // format lists of options and add appropriate indentation
  let formatList = items => items.map(item => Array.isArray(item) ?
  // indent items and separate columns by 2 spaces each
  formatOptionHelp(item, usageWidth + 2, maxWidth - 2) : item).join('\n').replace(/^(?!$)/gm, ' '.repeat(2));

  // format the command name to include the topmost command name
  ...
  output.push('Usage:', formatList([`$ ${commandUsage(command)}`]), '');

  // include command arguments
  if (args.length) output.push('Arguments:', formatList(args), '');

  // include command subcommands
  let commandGroupName = command.parent ? 'Subcommands:' : 'Commands:';
  if (commands.length) output.push(commandGroupName, formatList(commands), '');

  // separate flags into groups
  let groups = flags.reduce((all, option) => {
    let group = option[2] ? `${option[2]} options` : 'Options';
    return all.set(group, [...(all.get(group) || []), option]);
  }, new Map([['Options', []]]));

  // include each group of command flags
  ...
  if ((_examples2 = examples) !== null && _examples2 !== void 0 && _examples2.length) output.push('Examples:', formatList(examples), '');

  // join each line of output
  return output.join('\n');
}
