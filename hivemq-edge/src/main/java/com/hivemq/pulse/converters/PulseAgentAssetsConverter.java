...
public class PulseAgentAssetsConverter implements PulseApiEntityConverter<ManagedAssetList, PulseAgentAssets> {
    ...
    public @NotNull ManagedAssetList toRestEntity(final @NotNull PulseAgentAssets assets) {
        return ManagedAssetList.builder()
                .items(assets.stream().map(PulseAgentAssetConverter.INSTANCE::toRestEntity).toList())
                .build();
    }
}
