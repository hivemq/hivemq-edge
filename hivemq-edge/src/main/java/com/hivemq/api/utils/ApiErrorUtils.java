...
public class ApiErrorUtils {
    ...
    public static void validateRequiredField(
        ...
        if (Objects.isNull(fieldValue)) {
            apiErrorMessages.addError(
                    new ApiErrorMessage(fieldName, "Invalid user supplied data", "Supplied field was null"));
        } else if (!allowEmpty && "".equals(fieldValue.trim())) {
            apiErrorMessages.addError(
                    new ApiErrorMessage(fieldName, "Invalid user supplied data", "Supplied field was empty"));
        }
    }
    ...
    public static void validateFieldLengthBetweenIncl(
        ...
        if (value != null) {
            ...
            if (len < min || len > max) {
                apiErrorMessages.addError(new ApiErrorMessage(
                        fieldName,
                        String.format(
                                "Length of field value [%s] was outside the allowed bounds [%s, %s]", len, min, max),
                        null));
            }
        }
    }

    public static void validateFieldValueBetweenIncl(
            final ApiErrorMessages apiErrorMessages,
            final String fieldName,
            final int value,
            final int min,
        ...
        if (value < min || value > max) {
            apiErrorMessages.addError(new ApiErrorMessage(
                    fieldName,
                    String.format("Size of field value [%s] was outside the allowed bounds [%s, %s]", value, min, max),
                    null));
        }
    }
