...
/**
 * The ten states of the adapter machine. Apart from {@link #STOPPED}, {@link #CONNECTED}, and
 ...
 * acknowledgment-waiting state arms a watchdog on entry; {@link #WAITING_FOR_CONNECTION_RETRY} is
...
public enum ProtocolAdapterWrapperState implements FSMState {
    ...
    /**
     * {@code verifyBatch(...)} issued; waiting for every node to report a verification outcome.
     */
    ...
    CONNECTED,
    /**
     * Waiting out the connection backoff before the next {@code connect()}. The one waiting state
     ...
     * watchdog); waiting for {@code disconnected()} before backing off.
     */
