...
/**
 * The common query interface every aspect-machine state implements. A {@code FSM} needs exactly one
 ...
 * pre-operating states plus its role-specific states. This interface lets the read-side view fold
...
public interface TagAspectState extends FSMState {
     ...
     *         until a user-commanded tag retry.
     */
     ...
     *         counts as operating, so a tag does not flap to {@code ERROR} during a normal round-trip.
     */
    boolean isOperating();
}
