...
/**
 * A tag in the {@code <v2-protocol-adapters>} section (design §9.1) — one node the adapter exposes, plus the persisted
 * per-aspect activation preferences that carry the EDG-462 tag lifecycle (D7). The {@code node-string} is the opaque
 * JSON node definition, stored as-is and round-tripped to the adapter unchanged. {@code read-activated} and
 * {@code write-activated} default to {@code true} when absent (§9.1); a reload that flips only these flags is an
 * {@code ACTIVATION_ONLY} change applied without reconnecting (§8.2). Edge only reads this section; it is generated by
 * external tooling.
 */
