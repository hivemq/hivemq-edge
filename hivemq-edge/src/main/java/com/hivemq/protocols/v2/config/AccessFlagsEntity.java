 ...
 * element is PRESENT but one of its child attributes is omitted, that attribute defaults to
 * {@link AccessTriState#NO} (no declared capability). An entirely OMITTED {@code <access>} element is a distinct
 * case handled by {@link TagEntity}, whose field initializer is {@link #unrestricted()} (all effective
 * capabilities {@code YES}) for backward compatibility.
 */
