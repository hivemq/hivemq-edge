...
public class MqttsnToMqtt5Transcoder implements ITranscoder<IMqttsnMessage, Message> {
            ...
            switch (messageIn.getMessageType()) {
                case MqttsnConstants.CONNECT -> {
                    ...
                    if (!result.isComplete()) {
                        ...
                        connectBuilder
                                .withClientIdentifier(codec.getClientId(messageIn))
                ...
                case MqttsnConstants.PUBLISH, MqttsnConstants.PUBLISH_M1 -> {
                    ...
                    if (mqttsnPublish.getId() > 0) {
                        connection.correlatePublishToTopicAlias(
                                mqttsnPublish.getId(), mqttsnPublish.readTopicDataAsInteger());
                    }
                    final PUBLISH publish = new PUBLISHFactory.Mqtt5Builder()
                            ...
                            .withTopic(pipelineContext
                                    .getTopicRegistry()
                                    .readTopicName(
                                            connection.getClientId(),
                                            mqttsnPublish.getTopicType(),
                                            mqttsnPublish.getTopicData(),
                                            false))
                            .build();
                    out = publish;
                }
                case MqttsnConstants.PUBACK -> {
                    ...
                    final PUBACK puback = new PUBACK(
                            ...
                            mqttsnPuback.isErrorMessage()
                                    ? Mqtt5PubAckReasonCode.UNSPECIFIED_ERROR
                ...
                case MqttsnConstants.SUBSCRIBE -> {
                    ...
                    if (mqttsnSubscribe.getTopicType() == MqttsnConstants.TOPIC_SHORT) {
                        ...
                        topicName = pipelineContext
                                .getTopicRegistry()
                                .readTopicName(
                                        connection.getClientId(),
                                        mqttsnSubscribe.getTopicType(),
                ...
                case MqttsnConstants.UNSUBSCRIBE -> {
                    ...
                    String topicNameFromUnsubscribe = pipelineContext
                            ...
                            .readTopicName(
                                    connection.getClientId(),
                                    mqttsnUnsubscribe.getTopicType(),
                                    mqttsnUnsubscribe.getTopicData(),
                                    true);
    ...
    protected void preProcessSNConnect(
            NettyPipelineTranscodingContext ctx,
        ...
        int maxLength = codec.getProtocolVersion() == 1
                ? MqttsnConstants.MAX_CLIENT_ID_LENGTH_v12
                : MqttsnConstants.MAX_CLIENT_ID_LENGTH_v2;
        maxLength = Math.min(
                maxLength, ctx.getConfigurationService().mqttsnConfiguration().getMaxClientIdentifierLength());

        if (!MqttsnSpecificationValidator.validClientId(
                clientId,
            ...
            ctx.getMqttConnacker()
                    .connackError(
                            ctx.getChannelHandlerContext().channel(),
