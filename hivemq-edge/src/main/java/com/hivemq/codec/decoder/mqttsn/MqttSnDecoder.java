...
public class MqttSnDecoder extends ByteToMessageDecoder {
        ...
        try {
            ...
            while (inBuf.readableBytes() >= 2) { // 2 is min packet size
                ...
                if (logger.isTraceEnabled()) {
                    logger.trace(
                            "processing inbound {} bytes on connection {}",
                            bytesMesg.length,
                ...
                if (transcoder.canHandle(transcodingContext, mqttsnMessage.getClass())) {
                    ...
                    handled = result != null && result.isComplete();
                    if (handled && result != null) {
                        ...
                        if (result.isError()) {
                            logger.warn(
                                    "Error encountered during transcoding - {}",
     ...
     */
    protected void detectProtocolVersion(
            final @NotNull NettyPipelineTranscodingContext context, final @NotNull byte[] header) {
