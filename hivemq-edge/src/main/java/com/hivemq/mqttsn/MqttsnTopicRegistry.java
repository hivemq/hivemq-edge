 ...
 */
@Singleton
public class MqttsnTopicRegistry implements IMqttsnTopicRegistry {
    private static final Logger log = LoggerFactory.getLogger(MqttsnTopicRegistry.class);

    private final @NotNull MqttsnConfigurationService configurationService;

    private final Map<String, Map<Integer, MqttsnTopicAlias>> sessionRegistrations = new ConcurrentHashMap<>();
    ...
    public String readTopicName(
        ...
        if (topicIdType == MqttsnConstants.TOPIC_SHORT) {
            ...
            if (readNormalAsFullTopic) {
                ...
                if (log.isTraceEnabled()) {
                    log.trace(
                            "Considered topicData as String data for normal message (SUBSCRIBE) clientId {} -> {}",
                            clientId,
                            topicName);
                }
            } else {
                ...
                if (normalAliases != null) {
                    ...
                    if (registeredAlias != null) {
                        if (log.isTraceEnabled()) {
                            log.trace(
                                    "Found matching normal topic alias for clientId {} -> {}",
                                    clientId,
                                    registeredAlias);
                        }
        ...
        if (topicName == null) {
            log.warn(
                    "unable to determine topic from supplied alias configuration; clientId={}, topicTypeId={}, topicData={}",
                    clientId,
                    topicIdType,
                    MqttsnWireUtils.toHex(topicData));
    ...
    public Optional<MqttsnTopicAlias> readTopicAlias(@NotNull String clientId, @NotNull String topicName) {
        ...
        if (!configurationService.getPredefinedTopicAliases().isEmpty()) {
            Optional<MqttsnTopicAlias> alias = configurationService.getPredefinedTopicAliases().values().stream()
    ...
    public int register(@NotNull String clientId, @NotNull String topicName) throws MqttsnProtocolException {
        ...
        Map<Integer, MqttsnTopicAlias> sessionAlias = sessionRegistrations.get(clientId);
        if (sessionAlias == null) {
            sessionAlias = sessionRegistrations.get(clientId);
            if (sessionAlias == null) {
     ...
     */
    public static int getNextAvailableUint16(final @NotNull Collection<Integer> used, final int startAt)
            throws MqttsnProtocolException {
        if (used.isEmpty()) {
            return startAt;
        }
