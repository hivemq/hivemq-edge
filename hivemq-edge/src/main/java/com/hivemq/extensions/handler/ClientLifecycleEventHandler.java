...
public class ClientLifecycleEventHandler extends SimpleChannelInboundHandler<CONNECT> {
    ...
    private void fireOnServerDisconnect(
        ...
        for (final Map.Entry<String, ClientLifecycleEventListenerProvider> eventListenerEntry :
                pluginEventListenerProviderMap.entrySet()) {
    ...
    private void fireOnClientDisconnect(
        ...
        final PluginInTaskContext taskContext = new ProviderInTaskContext(clientId);
        final ClientInitiatedDisconnectInputImpl disconnectInput = new ClientInitiatedDisconnectInputImpl(
                clientId,
                ctx.channel(),
                disconnectEvent.getReasonCode(),
                disconnectEvent.getReasonString(),
    ...
    private void fireOnAuthFailed(
        ...
        final PluginInTaskContext taskContext = new ProviderInTaskContext(clientId);
        final AuthenticationFailedInputImpl failedInput = new AuthenticationFailedInputImpl(
                ctx.channel(),
                clientId,
    ...
    private void fireOnMqttConnect(final @NotNull ChannelHandlerContext ctx, final @NotNull CONNECT connect) {
        ...
        for (final Map.Entry<String, ClientLifecycleEventListenerProvider> eventListenerEntry :
                pluginEventListenerProviderMap.entrySet()) {
            final EventTask<ConnectionStartInputImpl> connectEventTask = new EventTask<>(
                    eventListenerEntry.getValue(), providerInput, eventListenerEntry.getKey(), eventListeners);
    ...
    private static class EventTask<T extends PluginTaskInput> implements PluginInTask<T> {
        ...
        public void accept(final @NotNull T pluginTaskInput) {
            ...
            try {
                ...
                } else if (pluginTaskInput instanceof AuthenticationSuccessfulInputImpl authSuccessfulInput) {
                    eventListener.onAuthenticationSuccessful(authSuccessfulInput);
        ...
        private @Nullable ClientLifecycleEventListener updateAndGetEventListener() {
            ...
            for (final Map.Entry<String, ClientLifecycleEventListener> pluginEventListenerEntry :
                    eventListeners.getPluginEventListenersMap().entrySet()) {
                ...
                if (listener.getClass()
                                .getClassLoader()
                                .equals(eventListenerProvider.getClass().getClassLoader())
                        && id.equals(pluginId)) {
                    contains = true;
