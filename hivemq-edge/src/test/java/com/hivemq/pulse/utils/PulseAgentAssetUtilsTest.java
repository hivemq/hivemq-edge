...
public class PulseAgentAssetUtilsTest {
    ...
    public void setUp() {
        ...
        localAssets = IntStream.range(0, STATUSES.size())
                .mapToObj(this::createLocalAsset)
                .toList();
        remoteAssets = IntStream.range(0, STATUSES.size())
                .mapToObj(this::createRemoteAsset)
                .toList();
    }

    private PulseAssetEntity createLocalAsset(final int index) {
        final UUID uuid = UUID.fromString("00000000-0000-0000-0000-"
    ...
    public void whenConfigWithoutLocalAssetsAndWithMappings_thenMappingsShouldBeCleared() {
        ...
        final PulseEntity newPulseEntity = pulseEntityArgumentCaptor.getValue();
        assertThat(newPulseEntity.getPulseAssetsEntity().getPulseAssetEntities())
                .isEqualTo(localAssets);
    }

    @Test
    public void whenConfigWithLocalAssetsWithoutLocalAssetsAndWithMappings_thenMappingsShouldBeCleared() {
        final PulseEntity pulseEntity = new PulseEntity();
        pulseEntity.getPulseAssetsEntity().getPulseAssetEntities().forEach(e -> e.getMapping()
    ...
    public void whenLocalMappingStatusIsStreamingAndRemoteAssetUpdated_thenStatusShouldBeRequiresRemapping() {
        ...
        final PulseAssetEntity asset =
                newPulseEntity.getPulseAssetsEntity().getPulseAssetEntities().getFirst();
    ...
    public void whenLocalMappingExistsAndRemoteAssetIsRemoved_thenStatusShouldBeUpdatedCorrectly() {
        IntStream.range(0, STATUSES.size()).forEach(i -> {
            ...
            final PulseEntity pulseEntity = new PulseEntity();
            final UUID mappingId = UUID.randomUUID();
            final PulseAssetEntity expectedAsset = localAssets
                    .getFirst()
                    .withMapping(PulseAssetMappingEntity.builder()
                            .id(mappingId)
                            .status(status)
            ...
            assertThat(newPulseEntity.getPulseAssetsEntity().getPulseAssetEntities())
                    .hasSize(1);
            final PulseAssetEntity asset = newPulseEntity
                    .getPulseAssetsEntity()
                    .getPulseAssetEntities()
                    .getFirst();
    ...
    public void whenLocalMappingExistsAndRemoteAssetIsUpdated_thenStatusShouldBeUpdatedCorrectly() {
        IntStream.range(0, STATUSES.size()).forEach(i -> {
            ...
            if (status == PulseAssetMappingStatus.STREAMING) {
                expectedDataCombiner = new DataCombiner(
                        UUID.randomUUID(),
                        "name",
                        "description",
                        List.of(),
                        List.of(new DataCombining(
                                expectedAsset.getMapping().getId(),
                                new DataCombiningSources(
                                        new DataIdentifierReference(
                                                "source-id", DataIdentifierReference.Type.PULSE_ASSET),
                                        List.of(),
                                        List.of()),
                                new DataCombiningDestination(
                                        expectedAsset.getId().toString(), "topic", "{}"),
                                List.of())));
            ...
            assertThat(asset.getTopic()).isEqualTo("updated/topic");
            assertThat(asset.getSchema()).isEqualTo("{ \"name\": \"Updated Name\" }");
            assertThat(asset.getMapping().getId())
                    .isEqualTo(expectedAsset.getMapping().getId());
            switch (status) {
