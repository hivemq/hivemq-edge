...
public class CombinersResourceImplUpdateCombinerTest extends AbstractCombinersResourceImplTest {
    ...
    public void whenConfigNotWritable_thenReturnsConfigWritingDisabledError() {
        ...
        try (final Response response = combinersApi.updateCombiner(combiner.getId(), combiner)) {
    ...
    public void whenReferenceTypeIsTopicFilter_thenReturnsOK() {
        ...
        try (final Response response = combinersApi.updateCombiner(combiner.getId(), combiner)) {
            ...
            verify(dataCombiningExtractor).updateDataCombiner(any());
        }
    }

    @Test
    public void whenUpdateReturnsFalse_thenReturnsDataCombinerNotFoundError() {
        final Combiner combiner = createCombiner(EntityType.ADAPTER, DataIdentifierReference.TypeEnum.TOPIC_FILTER);

        when(dataCombiningExtractor.getCombinerById(any())).thenReturn(Optional.of(toDataCombiner(combiner)));
        when(dataCombiningExtractor.updateDataCombiner(any())).thenReturn(false);
