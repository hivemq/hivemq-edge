...
public class PulseApiImplUpdateManagedAssetTest extends AbstractPulseApiImplTest {
    ...
    public void whenConfigNotWritable_thenReturnsConfigWritingDisabledError() {
        ...
        try (final Response response = pulseApi.updateManagedAsset(
                expectedAsset.getId(), PulseAgentAssetConverter.INSTANCE.toRestEntity(expectedAsset))) {
    ...
    public void whenMappingIdIsUsedByAsset_thenReturnsInvalidManagedAssetMappingIdError() {
        ...
        when(pulseAssetsEntity.getPulseAssetEntities())
                .thenReturn(List.of(
                        expectedAsset.toPersistence(),
                        expectedAsset
                                .withMapping(expectedAsset.getMapping().withId(mappingId))
                                .toPersistence()));
        try (final Response response = pulseApi.updateManagedAsset(
                id,
                PulseAgentAssetConverter.INSTANCE.toRestEntity(expectedAsset
    ...
    public void whenAssetExistsAndMappingDoesNotExist_thenReturnsOK() {
        ...
        try (final Response response = pulseApi.updateManagedAsset(
            ...
            assertThat(assetsArgumentCaptor.getValue().getPulseAssetsEntity().getPulseAssetEntities())
                    .hasSize(1);
            final PulseAssetEntity asset = assetsArgumentCaptor
                    .getValue()
            ...
            assertThat(asset.getDescription())
                    .as("Description cannot be changed.")
    ...
    public void whenAssetExistsAndMappingIdIsNull_thenReturnsOK() {
        when(statusProvider.getStatus())
                .thenReturn(
                        new Status(Status.ActivationStatus.ACTIVATED, Status.ConnectionStatus.CONNECTED, List.of()));
        final List<UUID[]> testCases = List.of(
                new UUID[] {null, null}, new UUID[] {null, UUID.randomUUID()}, new UUID[] {UUID.randomUUID(), null});
        testCases.forEach(uuids -> {
            ...
            try (final Response response = pulseApi.updateManagedAsset(
                    expectedAsset.getId(),
                    PulseAgentAssetConverter.INSTANCE.toRestEntity(
                            expectedAsset.withMapping(expectedAsset.getMapping().withId(newMappingId))))) {
                assertThat(response.getStatus()).isEqualTo(200);
            }
        });
    }
