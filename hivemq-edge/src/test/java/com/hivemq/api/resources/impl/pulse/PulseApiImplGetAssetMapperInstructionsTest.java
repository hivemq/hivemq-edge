...
public class PulseApiImplGetAssetMapperInstructionsTest extends AbstractPulseApiImplTest {
    ...
    public void whenCombinerAndMappingIdExist_thenReturnsOK() {
        ...
        try (final Response response = pulseApi.getAssetMapperInstructions(
                combiner.getId(), combiner.getMappings().getItems().getFirst().getId())) {
            assertThat(response.getStatus()).isEqualTo(200);
            assertThat(response.getEntity())
                    .isInstanceOf(List.class)
                    .isEqualTo(combiner.getMappings().getItems().stream()
