...
class LdapUsernameRolesProviderOpenLdapIntegrationTest {
    ...
    private static final String TEST_PASSWORD = "alice123";
    ...
    static void setUp() throws Exception {
        ...
        // "(&(objectClass=groupOfUniqueNames)(cn=administrators)(uniqueMember={userDn}))")
    ...
    void testBobHasBothAdminAndSuperRoles() {
        ...
        final Optional<IUsernameRolesProvider.UsernameRoles> bobResult =
                provider.findByUsernameAndPassword("bob", "bob456".getBytes(StandardCharsets.UTF_8));

        // Assert
        assertThat(bobResult)
                .as("Bob should authenticate with correct credentials")
                .isPresent();
        assertThat(bobResult.get().username()).isEqualTo("bob");
    ...
    void testCharlieHasNoRoles() {
        ...
        final Optional<IUsernameRolesProvider.UsernameRoles> charlieResult =
                provider.findByUsernameAndPassword("charlie", "charlie789".getBytes(StandardCharsets.UTF_8));

        // Assert
        assertThat(charlieResult)
                .as("Charlie should authenticate with correct credentials")
                .isPresent();
        assertThat(charlieResult.get().username()).isEqualTo("charlie");
    ...
    void testDonHasUserRole() {
        ...
        final Optional<IUsernameRolesProvider.UsernameRoles> donResult =
                provider.findByUsernameAndPassword("don", "don012".getBytes(StandardCharsets.UTF_8));

        // Assert
        assertThat(donResult)
                .as("Don should authenticate with correct credentials")
                .isPresent();
        assertThat(donResult.get().username()).isEqualTo("don");
