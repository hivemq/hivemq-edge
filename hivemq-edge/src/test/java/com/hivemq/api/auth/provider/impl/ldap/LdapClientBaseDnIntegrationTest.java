...
class LdapClientBaseDnIntegrationTest {
    ...
    static void setUp() throws Exception {
        ...
        new LdapTestConnection(setupProps)
                .createTestUser(LLDAP_CONTAINER.getAdminDn(), LLDAP_CONTAINER.getAdminPassword(), baseDn);
    }
    ...
    void testLegacyMode_absoluteRdns() throws Exception {
        ...
        try {
            ...
            final boolean authenticated =
                    client.authenticateUser(TEST_USERNAME, TEST_PASSWORD.getBytes(StandardCharsets.UTF_8));
    ...
    void testEquivalentConfigurations() throws Exception {
        ...
        final LdapConnectionProperties.LdapSimpleBind legacyBind = new LdapConnectionProperties.LdapSimpleBind(
                "uid=" + LLDAP_CONTAINER.getAdminUsername(), LLDAP_CONTAINER.getAdminPassword());
        ...
        final LdapConnectionProperties.LdapSimpleBind newBind = new LdapConnectionProperties.LdapSimpleBind(
                "uid=" + LLDAP_CONTAINER.getAdminUsername() + ",ou=people", // Relative to baseDn
                LLDAP_CONTAINER.getAdminPassword());
        ...
        try {
            ...
            final boolean legacyAuth =
                    legacyClient.authenticateUser(TEST_USERNAME, TEST_PASSWORD.getBytes(StandardCharsets.UTF_8));

            final boolean newAuth =
                    newClient.authenticateUser(TEST_USERNAME, TEST_PASSWORD.getBytes(StandardCharsets.UTF_8));

            assertThat(legacyAuth).as("Legacy mode authentication").isTrue();

            assertThat(newAuth).as("New mode authentication").isTrue();
