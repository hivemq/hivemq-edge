 ...
 * @since 4.0
 */
public class Mqtt5PubrecDecoderTest extends AbstractMqtt5DecoderTest {

    private final @NotNull Mqtt5PubRecReasonCode reasonCode = Mqtt5PubRecReasonCode.SUCCESS;
    ...
    public void decode_big_packet() {
        ...
        final PUBREC pubRec = decode(encoded);
        assertNotNull(pubRec);

        assertEquals(5, pubRec.getPacketIdentifier());

        assertEquals(reasonCode, pubRec.getReasonCode());

        assertEquals("success", pubRec.getReasonString());

        final ImmutableList<MqttUserProperty> userProperties = pubRec.getUserProperties().asList();
    ...
    public void decode_minimal_packet() {
        ...
        assertNull(pubRec.getReasonString());
        assertEquals(0, pubRec.getUserProperties().asList().size());
    }
    ...
    @NotNull
    private PUBREC decode(final byte @NotNull [] encoded) {
        final ByteBuf byteBuf = channel.alloc().buffer();
