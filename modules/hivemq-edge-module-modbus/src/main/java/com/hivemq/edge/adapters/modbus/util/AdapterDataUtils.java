...
public class AdapterDataUtils {
    ...
    public static @NotNull List<DataPoint> mergeChangedSamples(
        ...
        final Map<String, DataPoint> historicalSamplesMap =
                historicalSamples.stream().collect(Collectors.toMap(DataPoint::getTagName, Function.identity()));

        return currentSamples.stream()
                .filter(sample -> !(historicalSamplesMap.containsKey(sample.getTagName())
                        && historicalSamplesMap
                                .get(sample.getTagName())
                                .getTagValue()
                                .equals(sample.getTagValue())))
