...
class DatabasesProtocolAdapter2Test {
    ...
    @BeforeEach
    void setUp() {
        when(delegate.getId()).thenReturn("test-databases");
    ...
    void delegatesIdToWrappedAdapter() {
        final DatabasesProtocolAdapter2 adapter = new DatabasesProtocolAdapter2(delegate, moduleServices);
        assertThat(adapter.getId()).isEqualTo("test-databases");
    }

    @Test
    void factoryCreatesCorrectType() {
        final DatabasesProtocolAdapterFactory factory = new DatabasesProtocolAdapterFactory();
        final ProtocolAdapter2 result = factory.createProtocolAdapter2(delegate, moduleServices);
        assertThat(result).isInstanceOf(DatabasesProtocolAdapter2.class);
