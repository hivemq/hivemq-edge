...
class EipProtocolAdapter2Test {
    ...
    @BeforeEach
    void setUp() {
        when(delegate.getId()).thenReturn("test-eip");
        when(delegate.getProtocolAdapterInformation()).thenReturn(adapterInfo);
    }

    @Test
    void supportsSouthbound_returnsFalse() {
        final EipProtocolAdapter2 adapter = new EipProtocolAdapter2(delegate, moduleServices);
    ...
    void factoryCreatesCorrectType() {
        final EipProtocolAdapterFactory factory = new EipProtocolAdapterFactory(factoryInput);
        final ProtocolAdapter2 result = factory.createProtocolAdapter2(delegate, moduleServices);
        assertThat(result).isInstanceOf(EipProtocolAdapter2.class);
    }
}
