...
/**
 * The module-loader factory for the {@code "workload"} type. Each instance's {@link WorkloadScenario} is read from its
 * {@code <adapter-configuration>} at create time, so the adapter self-drives from its config with no in-JVM harness.
 */
public final class WorkloadProtocolAdapterFactory implements ProtocolAdapterFactory {
    ...
    public @NotNull Schema nodeDefinitionSchema() {
        return NODE_SCHEMA; // declare a real typed contract: DOUBLE in [0,100]
    }
}
