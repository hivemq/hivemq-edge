...
public class RedisProtocolAdapterInformation implements ProtocolAdapterInformation {

    public static final @NotNull ProtocolAdapterInformation INSTANCE = new RedisProtocolAdapterInformation();
    private static final @NotNull Logger LOG = LoggerFactory.getLogger(RedisProtocolAdapterInformation.class);

    protected RedisProtocolAdapterInformation() {
    }
    ...
    public @Nullable String getUiSchema() {
        try (final InputStream is = this.getClass()
                .getClassLoader()
                .getResourceAsStream("redis-adapter-ui-schema.json")) {
            if (is == null) {
                LOG.warn("The UISchema for the Redis Adapter could not be loaded from resources: Not found.");
                return null;
            }
