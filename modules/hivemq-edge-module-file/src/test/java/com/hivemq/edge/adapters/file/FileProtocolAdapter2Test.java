...
class FileProtocolAdapter2Test {
    ...
    @BeforeEach
    void setUp() {
        when(delegate.getId()).thenReturn("test-file");
        when(delegate.getProtocolAdapterInformation()).thenReturn(adapterInfo);
    }

    @Test
    void supportsSouthbound_returnsFalse() {
        final FileProtocolAdapter2 adapter = new FileProtocolAdapter2(delegate, moduleServices);
    ...
    void factoryCreatesCorrectType() {
        final FileProtocolAdapterFactory factory = new FileProtocolAdapterFactory(factoryInput);
        final ProtocolAdapter2 result = factory.createProtocolAdapter2(delegate, moduleServices);
        assertThat(result).isInstanceOf(FileProtocolAdapter2.class);
    }
}
