...
/**
 * Test seams for driving a {@link HttpProtocolAdapter} directly: a {@link ProtocolAdapterInput} whose adapter
 * configuration carries a supplied settings map (so a test can flip {@code assertResponseIsJson} /
 * {@code httpPublishSuccessStatusCodeOnly}), and a {@link DataPointFactory} whose {@link TestDataPoint} records the
 * value and its JSON flag.
 */
public final class HttpAdapterTestFixtures {

    private HttpAdapterTestFixtures() {}
     ...
     * @param config     the adapter configuration settings.
     * @return a {@link ProtocolAdapterInput} carrying the given configuration and no pre-declared nodes (poll tests
    ...
    public static @NotNull ProtocolAdapterInput input(
        ...
        final DataPoint configDataPoint = factory.createJsonDataPoint(adapterId, config);
    ...
    /**
     * A {@link DataPointFactory} that builds {@link TestDataPoint}s, preserving the JSON flag.
     */
