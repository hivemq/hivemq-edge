...
// The v2 HTTP(s) adapter is a production, listable SDK v2 adapter. The HTTP client is the JDK's own
// java.net.http.HttpClient — there is no third-party HTTP dependency — so the module bundles no runtime dependencies
// (the host provides the adapter SDK and Jackson) and its shadow jar carries only its own classes, the v2
// ProtocolAdapterFactory service file, and the logo. It is loadable through the standard module loader exactly as the
// v1 HTTP adapter is.

dependencies {
    compileOnly(libs.hivemq.edge.adaptersdk)
    // Jackson: HttpNode carries a @JsonCreator so the framework's own ObjectMapper deserializes it from its
    // node-string, the adapter configuration is parsed with it, and JSON responses are parsed with it. Jackson is
