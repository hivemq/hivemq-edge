...
class ADSProtocolAdapter2Test {
    ...
    @BeforeEach
    void setUp() {
        when(delegate.getId()).thenReturn("test-ads");
        when(delegate.getProtocolAdapterInformation()).thenReturn(adapterInfo);
    }

    @Test
    void supportsSouthbound_returnsFalse() {
        final ADSProtocolAdapter2 adapter = new ADSProtocolAdapter2(delegate, moduleServices);
    ...
    void delegatesIdToWrappedAdapter() {
        final ADSProtocolAdapter2 adapter = new ADSProtocolAdapter2(delegate, moduleServices);
        assertThat(adapter.getId()).isEqualTo("test-ads");
    }

    @Test
    void factoryCreatesCorrectType() {
        final ADSProtocolAdapterFactory factory = new ADSProtocolAdapterFactory(factoryInput);
        final ProtocolAdapter2 result = factory.createProtocolAdapter2(delegate, moduleServices);
        assertThat(result).isInstanceOf(ADSProtocolAdapter2.class);
