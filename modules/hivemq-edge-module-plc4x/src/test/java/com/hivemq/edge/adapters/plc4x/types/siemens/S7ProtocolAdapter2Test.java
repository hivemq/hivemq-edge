...
class S7ProtocolAdapter2Test {
    ...
    @BeforeEach
    void setUp() {
        when(delegate.getId()).thenReturn("test-s7");
    ...
    void delegatesIdToWrappedAdapter() {
        final S7ProtocolAdapter2 adapter = new S7ProtocolAdapter2(delegate, moduleServices);
        assertThat(adapter.getId()).isEqualTo("test-s7");
    }

    @Test
    void factoryCreatesCorrectType() {
        final S7ProtocolAdapterFactory factory = new S7ProtocolAdapterFactory(factoryInput);
        final ProtocolAdapter2 result = factory.createProtocolAdapter2(delegate, moduleServices);
        assertThat(result).isInstanceOf(S7ProtocolAdapter2.class);
