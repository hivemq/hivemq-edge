...
The backend must implement validation for:
...
The adapter schema layer has a number of known defects identified in a 2025 audit — covering field naming inconsistencies, missing validation, incomplete `$schema` declarations, and UI rendering edge cases. Issues span both the frontend (missing format validators, incorrect mock schemas, orphaned widgets) and the backend (wrong getter implementations, missing conditional field dependencies, raw enum values without display names).

**See:** [Adapter Schema Analysis 2025](../analysis/ADAPTER_SCHEMA_ANALYSIS_2025.md) for the full inventory of 28 issues and their recommended fixes.
