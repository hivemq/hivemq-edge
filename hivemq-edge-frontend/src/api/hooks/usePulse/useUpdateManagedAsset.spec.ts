...
describe('useUpdateManagedAsset', () => {
  afterEach(() => {
  ...
  it('should execute the mutation', async () => {
    ...
    const { result } = renderHook(useUpdateManagedAsset, { wrapper })

    expect(result.current.isSuccess).toBeFalsy()
    await act(async () => {
      await result.current.mutateAsync({
        assetId: MOCK_PULSE_ASSET_MAPPED.id,
        requestBody: { ...MOCK_PULSE_ASSET_MAPPED, name: 'The new name' },
      })
    })
    await waitFor(() => {
    ...
    expect(result.current.data).toStrictEqual({
      updated: '3b028f58-f949-4de1-9b8b-c1a35b1643a5',
      data: 'The new name',
    })
  })
})
