...
export const formatScriptName = (functionNode: Node<FunctionData>, withPrefix = true): string => {
  const version =
    functionNode.data.version === ResourceWorkingVersion.DRAFT ||
    functionNode.data.version === ResourceWorkingVersion.MODIFIED
      ? SCRIPT_FUNCTION_LATEST
      : functionNode.data.version

  const shortName = `${functionNode.data.name}:${version}`
  const prefixedName = `${SCRIPT_FUNCTION_PREFIX}:${shortName}`
  return withPrefix ? prefixedName : shortName
}
...
export function checkValidityJSScript(scriptNode: Node<FunctionData>): DryRunResults<Script> {
  ...
  const script: Script = {
    ...
    description: scriptNode.data.description,
  ...
  for (const fct of functions) {
    ...
    const functionScriptNode: Node<FunctionData> = {
      ...
      data: {
        ...
        description: functionScript.description,
