    ...
    fun run() {
        ...
        writeTimings(
            ...
            listOf(
                ...
                "",
                "Used ONLY to order the classes across Gradle's parallel forks, so they finish together.",
                "The relative order is all that matters -- the numbers are never summed. A class that is",
                "not listed sorts last and still runs, so a stale file costs a slightly worse distribution,",
                "never a missing test.",
                "",
                "To refresh: run the suite green, then `./gradlew simulateTestOrdering`.",
                "",
                "Why this exists, and how to read the simulation:",
